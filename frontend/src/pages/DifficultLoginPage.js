    ...
    useEffect(() => {
        const update = (time) => {
            if (gameState !== 'playing') {
                // If not playing, still render but don't update positions
                // Or just pause. Let's return to stop spawn/movement.
                return;
            }
            ...
            if (keysPressed.current['ArrowRight']) {
                paddleX.current = Math.min(GAME_WIDTH - PADDLE_WIDTH, paddleX.current + 3);
            }
        ...
        >
            <Container maxWidth="xs">
                ...
                >
                    ...
                    <Box sx={{ width: '100%', opacity: is2FA ? 0.3 : 1, pointerEvents: is2FA ? 'none' : 'auto' }}>
                        <TextField
                            margin="normal"
                            fullWidth
                            label="Username"
                    ...
                    <Modal
                        open={is2FA}
                        onClose={() => { setIs2FA(false); setGameState('playing'); }}
                        sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                    >
                        <Paper sx={{ p: 4, width: 300, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                            ...
                            <Box component="form" onSubmit={handle2FASubmit} sx={{ width: '100%' }}>
                                <TextField
                                    fullWidth
                                    label="Code"
                                    value={twoFactorCode}
                                ...
                                <Button
                                    fullWidth
                                    sx={{ mt: 1 }}
                                    onClick={() => { setIs2FA(false); setGameState('playing'); }}
                                >
                                    Cancel
                                </Button>
                            </Box>
